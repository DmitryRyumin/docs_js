...
| `npm outdated --depth=0`<br>`npm outdated -g --depth=0`  | Список локальных пакетов, которые необходимо обновить<br>Список глобальных пакетов, которые необходимо обновить                         |
...
| `lite-server`   | Автоматический перезапуск сервера при обнаружении изменений файлов в каталоге |
...
```javascript
let normalize = path.normalize(path.join(__dirname, '/////test', 'second.html'))
```
...
```javascript
// Создание файла
fs.writeFile(path.join(__dirname, 'notes', 'mynotes.txt'), 'Привет мир!', (err) => {
  if (err) throw err
...
```javascript
// Дозапись в файл
fs.appendFile(path.join(__dirname, 'notes', 'mynotes.txt'), ' Текст', (err) => {
  if (err) throw err
